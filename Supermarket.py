...
if choice == 1 :
	...
	if len(items) != 0:
		...
		print('Items has been successfully added.')
		items.append(item)
		...
		for item in items:
			if item_name.lower() == item['name'].lower():
				print('Here are the current details of ' + find_item)
				print(item)
				item['name'] = input('Item name : ')
				while True:
					try:
				...
				while True:
					try:
						...
						print('Price should only be in digits')
				print('Item has been successfully update.')
				print(item)
			else:
				print('Item not found')
